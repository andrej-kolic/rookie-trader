...
/**
 * Calculate depth percentages for price levels based on max total
 * Used for visualizing depth bars in the order book display
 *
 * @param levels Price levels with cumulative totals
 * @param maxTotal Maximum cumulative total (reference point)
 * @returns Array of depth percentages (0-100)
 */
export function calculateDepthPercentages(
  levels: readonly OrderBookLevel[],
  maxTotal: number,
): number[] {
  if (maxTotal === 0) return levels.map(() => 0);

  return levels.map((level) => (level.total / maxTotal) * 100);
}

/**
 * Validates that price levels are correctly sorted for their side
 * Bids should be descending (highest first), asks should be ascending (lowest first)
 *
 * @param levels Price levels to validate
 * @param side Whether these are bids or asks
 * @returns true if correctly sorted, false otherwise
 */
export function validateLevelsSorting(
  levels: readonly OrderBookLevel[],
  side: OrderBookSide,
): boolean {
  ...
  for (let i = 0; i < levels.length - 1; i++) {
    ...
    if (side === 'bid') {
      // Bids should be descending (current >= next)
      if (current.price < next.price) return false;
    } else {
      // Asks should be ascending (current <= next)
      if (current.price > next.price) return false;
    }
  }
...
/**
 * Validates that spread is positive (best ask > best bid)
 *
 * @param bestBid Highest bid price
 * @param bestAsk Lowest ask price
 * @returns true if spread is valid (positive or zero), false if negative
 */
